...
# common layers
from arch.abstract_net_cascaded import AbstractNetwork
import arch.layer_cascaded as lyr
...
class Network(AbstractNetwork):
  ...
  def build_graph(self, img_pl, train_stage, train_stage_cascaded, data_format="NCHW"):
    ...
    if data_format == "NCHW":
      logits_decoder = tf.transpose(logits_linear, [0, 2, 3, 1]) # NEW: RENAMED logits -> logits_decoder
    else:
      logits_decoder = logits_linear # NEW: RENAMED logits -> logits_decoder

    # NEW:
    #####################################

    # newly add cascaded layer
    with tf.variable_scope("cascaded"):
      print("cascaded")
      # OLD 1 Linear Classification layer after Unpool2
      # with tf.variable_scope("logits_pre"):
      #   print("logits_pre")
      #   logits_linear_cas_pre = lyr.linear_layer(un_non_bt_4_lyr2, self.num_classes,
      #                                    train_stage_cascaded and train_lyr[8],
      #                                    summary=summary,
      #                                    data_format=data_format)

      # 1 Downsampled Linear Classification layer from decoded
      with tf.variable_scope("logits_pre"):
        print("logits_pre")
        logits_linear_cas_pre = lyr.max_pool(logits_linear, data_format=data_format)

      # 2 cascade logits_linear_cas with features from 1st encoder output
      with tf.variable_scope("concat"):
        print("concat")
        if data_format == "NCHW":
          cascade_concat = tf.concat([logits_linear_cas_pre, non_bt_2_lyr1], 1)
        else:
          cascade_concat = tf.concat([logits_linear_cas_pre, non_bt_2_lyr1], 3)

      # 3 unpool + non-bt
      with tf.variable_scope("unpool3"):
        print("unpool3")
        # input image 256*128 - 240*180
        unpool_lyr3_cas = lyr.upsample_layer(cascade_concat,
                                         train_stage_cascaded and train_lyr[7],
                                         kernels=n_k_lyr[5],
                                         data_format=data_format)
        with tf.variable_scope("non-bt-1"):
          un_non_bt_1_lyr3_cas = lyr.uERF_non_bt(unpool_lyr3_cas, 3,
                                             train_stage_cascaded and train_lyr[7],
                                             summary,
                                             data_format=data_format,
                                             dropout=self.NET["dropout"],
        ...
        with tf.variable_scope("non-bt-2"):
          un_non_bt_2_lyr3_cas = lyr.uERF_non_bt(un_non_bt_1_lyr3_cas, 3,
                                             train_stage_cascaded and train_lyr[7],
                                             summary,
                                             data_format=data_format,
                                             dropout=self.NET["dropout"],
      ...
      # 4 Linear Classification layer after
      with tf.variable_scope("logits_post"):
        print("logits_post")
        logits_linear_cas_post = lyr.linear_layer(un_non_bt_2_lyr3_cas, self.num_classes,
                                             train_stage_cascaded and train_lyr[8],
                                             summary=summary,
                                             data_format=data_format)
      # transpose logits back to NHWC
      if data_format == "NCHW":
        logits_linear_cas = tf.transpose(logits_linear_cas_post, [0, 2, 3, 1])
      else:
        logits_linear_cas = logits_linear_cas_post
    ####################################

    # add delta logits ################
    with tf.variable_scope("logits_sum"):
      logits_sum = logits_decoder+logits_linear_cas

    logits = logits_decoder
    # END NEW

    return logits, logits_sum, code, n_img
