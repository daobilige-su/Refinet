...
class AbstractNetwork:
  ...
  # NEW:
  def build_graph(self, train_stage, train_stage_cascaded, data_format="NCHW"):
    # END NEW:
  ...
  def restore_session(self, path):
    ...
    if self.ckpt and self.ckpt.model_checkpoint_path:
      ...
      try:
        ...
        # NEW:
        if not self.fine_tune_on:
          not_restore.extend(tf.global_variables(scope='model/cascaded'))
          not_restore.extend(tf.global_variables(scope='model/logits_sum'))
        # END NEW:
  ...
  def predict_kickstart(self, path, batchsize=1, data_format="NCHW"):
    ...
    with tf.name_scope("test_model"):
      with tf.variable_scope("model", reuse=None):
        # NEW:
        logits_decoder, logits_sum, self.code_valid, self.n_img_valid = self.build_graph(
            self.img_pl, False, False, data_format=data_format)  # not training
        self.logits_valid = logits_sum
        # END NEW:
        ...
        with tf.variable_scope("model"):
          for i in range(self.n_gpus):
            with tf.device(self.assign_to_device('/gpu:%d' % i)):
              ...
              # graph
              # NEW:
              if self.fine_tune_on:
                logits_decoder, logits_sum, _, _ = self.build_graph(img_pl, True, True)  # train
              else:
                logits_decoder, logits_sum, _, _ = self.build_graph(img_pl, False, True)  # train
              logits_train = logits_sum
              # END NEW:
        ...
        with tf.variable_scope("model", reuse=True):
          for i in range(self.n_gpus):
            with tf.device('/gpu:%d' % i):
              ...
              # NEW:
              logits_decoder, logits_sum, _, _ = self.build_graph(img_pl, False, False)  # test
              logits_valid = logits_sum
              # END NEW:
